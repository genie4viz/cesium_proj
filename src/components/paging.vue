<template>
  <div class="paging clearfix">
    <div style="color: #b4d8f3; margin-bottom: 8px">显示第{{startnum}}到{{endnum}}条记录 共{{total}}条记录</div>
    <!--<div class="page-size fl" v-if="isShowTotal">共{{total}}条</div>-->
    <ul class="page-list fl clearfix">
      <li @click="changePage(currentPage-1)" style="margin-right: 2px"><</li>
      <li :class="{'active':currentPage==item.val}" v-for="item in pagelist" v-text="item.text" @click="changePage(item.val)" style="margin-right: 2px">1</li>
      <li @click="changePage(currentPage+1)" style="margin-right: 2px">></li>
    </ul>
  </div>
</template>
