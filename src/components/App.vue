  ...
  methods: {
    ...
    draw_all_el_towers(towers, tower, blink_flag) {  
      ...
      Cesium.sampleTerrainMostDetailed(
          ...
          for (var i = 0; i < towers.length; i++) {
            ...
            for (let k = 0; k < currentModel.units.length-1; k++) {
              ...
              if(i > 0 && (currentModel.units[k].type == 0 || currentModel.units[k].type == 1)) {
                ...
                line.push(pt2[1]);    line.push(pt2[0]);    line.push(currentModel.units[k].z_b_dist + 1.0 * towers[i].z_pos);
                lines.push(line);
          ...
          console.log(main.el_towers_on_line, 'main.el_towers_on_line!!!433333')
          for(let p=0; p<lines.length; p++) {
            viewer.entities.add({
              id: "el_tower_line_" + p,
              polyline: {
                positions: new Cesium.Cartesian3.fromDegreesArrayHeights(lines[p]),
